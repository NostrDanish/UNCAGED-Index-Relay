...
describe("OpenSearchRelay", () => {
  ...
  describe("NIP-50 domain filter", () => {
    // Mock client that supports domain resolution queries (kind 0 aggregations)
    // and normal search queries with author filtering.
    const createDomainMockClient = () => {
      const documents = new Map<string, unknown>();
      ...
      return {
        ...
        client: {
          search: async ({ body }: { body: Record<string, unknown> }) => {
            ...
            // Check if this is a domain resolution query (has aggregations + kind:0)
            if (body.aggs) {
              const aggs = body.aggs as Record<string, unknown>;
              if (aggs.pubkeys) {
                // Extract the domain field + value from the query
                let domainField: string | undefined;
                let domainValue: string | undefined;
                for (const clause of queryMust || []) {
                  const term = clause.term as Record<string, unknown>;
                  if (term?.nip05_domain) {
                    domainField = "nip05_domain";
                    domainValue = term.nip05_domain as string;
                  }
                  if (term?.nip05_hostname) {
                    domainField = "nip05_hostname";
                    domainValue = term.nip05_hostname as string;
                  }
                }

                // Collect matching pubkeys from kind 0 docs
                ...
                for (const [, doc] of documents.entries()) {
                  ...
                  if (
                    ...
                    !d.deleted &&
                    domainField &&
                    d[domainField] === domainValue
                  ) {
                    matchingPubkeys.add(d.pubkey as string);
                  }
                }
                ...
                return {
                  body: {
                    aggregations: {
                      pubkeys: {
                        buckets: [...matchingPubkeys].map((pk) => ({
                          key: pk,
                        })),
                      },
                    },
            ...
            // Normal search query — extract filters
            let authorFilter: string[] | undefined;
            let kindFilter: Set<number> | undefined;
            for (const clause of queryMust || []) {
              const terms = clause.terms as Record<string, unknown>;
              if (terms?.pubkey) {
                authorFilter = terms.pubkey as string[];
              }
              if (terms?.kind) {
                kindFilter = new Set(terms.kind as number[]);
              }
            }

            const results: unknown[] = [];
            for (const [, doc] of documents.entries()) {
              const d = doc as NostrEvent & { deleted?: boolean };
              if (d.deleted) continue;
              if (authorFilter && !authorFilter.includes(d.pubkey)) continue;
              if (kindFilter && !kindFilter.has(d.kind)) continue;
              results.push({ _source: doc });
            }
    ...
    it("should store nip05_domain and nip05_hostname on indexed documents", async () => {
      const { client, documents } = createDomainMockClient();
      const relay = new OpenSearchRelay(client as unknown as Client, {
        indexName: "test-index",
        bulkMaxSize: 1,
      });
      ...
      assert.equal(doc.nip05_domain, "example.com");
      assert.equal(doc.nip05_hostname, "example.com");
    });

    it("should filter events by bare domain (includes subdomains)", async () => {
      const { client } = createDomainMockClient();
      const relay = new OpenSearchRelay(client as unknown as Client, {
        indexName: "test-index",
        bulkMaxSize: 1,
      });
      ...
      // User on example.com (bare domain)
      const meta1 = finalizeEvent(
        {
          kind: 0,
          created_at: now,
          tags: [],
      ...
      // User on hi.example.com (subdomain)
      const meta2 = finalizeEvent(
        {
          kind: 0,
          created_at: now,
          tags: [],
          content: JSON.stringify({
            name: "bob",
            nip05: "bob@hi.example.com",
          }),
        },
        sk2,
      );

      // User on other.com (different domain)
      const meta3 = finalizeEvent(
        {
          kind: 0,
          created_at: now,
          tags: [],
          content: JSON.stringify({
            name: "carol",
            nip05: "carol@other.com",
          }),
        },
        sk3,
      );

      await relay.event(meta1, {
        analysis: {
          nip05_domain: "example.com",
          nip05_hostname: "example.com",
        },
      });
      await relay.event(meta2, {
        analysis: {
          nip05_domain: "example.com",
          nip05_hostname: "hi.example.com",
        },
      });
      await relay.event(meta3, {
        analysis: {
          nip05_domain: "other.com",
          nip05_hostname: "other.com",
        },
      });

      // Create kind 1 notes from each user
      const note1 = finalizeEvent(
        { kind: 1, created_at: now, tags: [], content: "Hello from alice" },
        sk1,
      );
      const note2 = finalizeEvent(
        { kind: 1, created_at: now, tags: [], content: "Hello from bob" },
        sk2,
      );
      const note3 = finalizeEvent(
        { kind: 1, created_at: now, tags: [], content: "Hello from carol" },
        sk3,
      );

      await relay.event(note1);
      await relay.event(note2);
      await relay.event(note3);

      // domain:example.com should match both alice and bob (bare domain + subdomain)
      const results = await relay.query([
        { kinds: [1], search: "domain:example.com" },
      ]);
      assert.equal(results.length, 2);
      const pubkeys = new Set(results.map((e) => e.pubkey));
      assert.ok(pubkeys.has(note1.pubkey));
      assert.ok(pubkeys.has(note2.pubkey));
    });

    it("should filter events by subdomain (exact match only)", async () => {
      const { client } = createDomainMockClient();
      const relay = new OpenSearchRelay(client as unknown as Client, {
        indexName: "test-index",
        bulkMaxSize: 1,
      ...
      // Create kind 1 notes
      const note1 = finalizeEvent(
        { kind: 1, created_at: now, tags: [], content: "From hi.example.com" },
        sk1,
      );
      const note2 = finalizeEvent(
        { kind: 1, created_at: now, tags: [], content: "From example.com" },
        sk2,
      );
      ...
      // domain:hi.example.com should match only alice (exact subdomain match)
      const results = await relay.query([
        { kinds: [1], search: "domain:hi.example.com" },
      ]);
      assert.equal(results.length, 1);
      assert.equal(results[0].pubkey, note1.pubkey);
    });

    it("should return empty results when domain matches no pubkeys", async () => {
      const { client } = createDomainMockClient();
      const relay = new OpenSearchRelay(client as unknown as Client, {
        indexName: "test-index",
        bulkMaxSize: 1,
      ...
      // No kind 0 events with nip05 data, so domain: should return empty
      const results = await relay.query([
        { kinds: [1], search: "domain:nonexistent.com" },
      ]);
      assert.equal(results.length, 0);
    });

    it("should intersect domain pubkeys with existing authors filter", async () => {
      const { client } = createDomainMockClient();
      const relay = new OpenSearchRelay(client as unknown as Client, {
        indexName: "test-index",
        bulkMaxSize: 1,
      ...
      // Create notes from both
      const note1 = finalizeEvent(
        { kind: 1, created_at: now, tags: [], content: "Alice's note" },
        sk1,
      );
      const note2 = finalizeEvent(
        { kind: 1, created_at: now, tags: [], content: "Bob's note" },
        sk2,
      );
      ...
      // Filter by domain AND specific author — should only return alice's note
      const results = await relay.query([
        {
          kinds: [1],
          authors: [meta1.pubkey],
    ...
    it("should return empty when authors and domain have no intersection", async () => {
      const { client } = createDomainMockClient();
      const relay = new OpenSearchRelay(client as unknown as Client, {
        indexName: "test-index",
        bulkMaxSize: 1,
      ...
      // Filter by domain:example.com but with author=bob — no intersection
      const results = await relay.query([
        {
          kinds: [1],
          authors: [note2.pubkey],
          search: "domain:example.com",
        },
      ]);
      assert.equal(results.length, 0);
    ...
    it("should work without search filter (no domain: token)", async () => {
      const { client } = createDomainMockClient();
      const relay = new OpenSearchRelay(client as unknown as Client, {
        indexName: "test-index",
        bulkMaxSize: 1,
      ...
      // Query without search — should still work normally
      const results = await relay.query([{ kinds: [1] }]);
      assert.equal(results.length, 1);
