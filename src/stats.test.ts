...
import { getTrendingTagValues } from "./stats.ts";
...
describe("getTrendingTagValues", () => {
  it("should return trending hashtags sorted by authors desc", async () => {
    const { client, addEvent } = createMockClient();
    ...
    const results = await getTrendingTagValues(client, "test-index", ["t"], {
      kinds: [1],
      since: now - 3600,
      until: now,
      limit: 10,
  ...
  it("should restrict to allowed values when provided", async () => {
    ...
    const results = await getTrendingTagValues(
      client,
      "test-index",
      ["t"],
  ...
  it("should work with trending pubkeys (p-tag)", async () => {
    ...
    const results = await getTrendingTagValues(client, "test-index", ["p"], {
      kinds: [1, 3, 6, 7, 9735],
      since: now - 3600,
      until: now,
      limit: 40,
